...
import {
  Heart, Bookmark, Clock, MapPin, ChevronRight,
  ArrowLeft, Sparkles, Home, Compass, Users, User, Navigation,
  ...
  return (
    <Pressable style={styles.fyCard} onPress={() => {}}>
      ...
      <View style={[styles.fyPhoto, { backgroundColor: item.color }]}>
        ...
        {/* Gradient scrim is tricky in RN without expo-linear-gradient — use a semi-transparent overlay */}
      ...
      <View style={styles.fyInfo}>
        ...
        <View style={styles.fyMeta}>
          <View style={styles.fyMetaLeft}>
            ...
            <Text style={styles.fyNeighborhood}>{item.neighborhood}</Text>
          </View>
...
function ScoutCard({ onPress }) {
  const { name, vibeTags, stopCount, stops } = SCOUT_SUGGESTION;
  return (
    <Pressable style={styles.scoutCard} onPress={onPress}>
      ...
      <View style={styles.scoutHeader}>
        ...
        />
        {/* Text block */}
        <View style={styles.scoutTextBlock}>
          <View style={styles.scoutLabelRow}>
            <AmberDot />
            <Text style={styles.scoutLabelText}>SCOUT'S PICK</Text>
          </View>
          <Text style={styles.scoutOutingName} numberOfLines={1}>{name}</Text>
          {/* Vibe tags */}
  ...
  return (
    <View style={styles.sectionBlock}>
      ...
      <FlatList
        ...
        snapToInterval={168} // card width 158 + gap 10
...
function OutingDraftCard({ onPress }) {
    ...
    <>
      ...
      {OTHER_DRAFT_COUNT >= 1 && (
        <Pressable style={styles.draftListLink} onPress={() => {}}>
          ...
          <ChevronRight size={12} color={C.textTert} />
        </Pressable>
      )}
...
function ActiveOutingCard({ onNextStop, onSeeDetails }) {
    ...
    <View style={styles.screen}>
      ...
      <ScrollView
        ...
        contentContainerStyle={[
          ...
          { paddingTop: insets.top + 18, paddingBottom: 100 },
        ]}
      ...
      >
        ...
        {homeState === 'A' && (
          <ScoutCard onPress={() => {/* navigate to Outing Preview */}} />
        )}

        {/* ── STATE B: Draft + Scout also suggests ── */}
        {homeState === 'B' && (
          <>
            <OutingDraftCard onPress={() => {/* navigate to Outing Preview/Edit */}} />
...
const styles = StyleSheet.create({
  ...
  fyScrim: {
    ...
    // Real scrim: use expo-linear-gradient <LinearGradient> for better quality
  },
  fyCatPill: {
    position: 'absolute',
    top: 8,
