/**
 * FORAGE — AddFriendInfoSheet
 * app/_add-friend-info-sheet.tsx
 *
 * Compact bottom-sheet shown when the user taps the Add Friend button on
 * the Friends tab. Same visual/motion pattern as _duo-plan-info-sheet.tsx
 * (slide-up sheet, 300ms ease-out, backdrop fade) — purely informational,
 * not a data-loss warning, so the backdrop is tappable and dismisses it
 * the same way the "Got it" button does.
 */
...
export interface AddFriendInfoSheetProps {
  visible: boolean;
  onDismiss: () => void;
}

export default function AddFriendInfoSheet({
  visible,
  onDismiss,
}: AddFriendInfoSheetProps) {
    ...
    <Modal visible={rendered} transparent animationType="none" onRequestClose={onDismiss}>
      <View style={styles.root}>
        ...
        >
          ...
          <Text style={styles.scoutLine}>Adding friends isn't wired up yet.</Text>

          <Text style={styles.progressLine}>Send and accept real friend requests. Not built yet. Ask me again later.</Text>
